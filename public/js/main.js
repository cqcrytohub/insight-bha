    ...
    function($scope, $rootScope, Currency) {
        ...
        $rootScope.currency.getConvertion = function(value) {
            ...
            if (!isNaN(value) && typeof value !== 'undefined' && value !== null) {
                ...
                } else if (this.symbol === APP_CONFIG.SYMBOL_M) {
                    this.factor = 1000;
                    response = _roundFloat((value * this.factor), 5);
                } else if (this.symbol === APP_CONFIG.SYMBOL_BITS) {
                    this.factor = 1000000;
                    response = _roundFloat((value * this.factor), 2);
        ...
        $scope.setCurrency = function(currency) {
            ...
            localStorage.setItem('insight-currency' + APP_CONFIG.SYMBOL, currency);

            if (currency === 'USD') {
                Currency.get({}, function(res) {
                    $rootScope.currency.factor = $rootScope.currency.bitstamp = res.data.bitstamp;
                });
            } else if (currency === APP_CONFIG.SYMBOL_M) {
                $rootScope.currency.factor = 1000;
            } else if (currency === APP_CONFIG.SYMBOL_BITS) {
...
angular.module('insight.system').controller('ScannerController',
    function($scope, $rootScope, $modalInstance, Global) {
        ...
        qrcode.callback = function(data) {
            ...
            var str = (data.indexOf(APP_CONFIG.URL + ':') === 0) ? data.substring(8) : data;
...
/* jshint -W100 */
    ...
    gettextCatalog.setStrings('ko', {"(Input unconfirmed)":"(미승인 입력)","404 Page not found :(":"404 페이지 찾을수 없음","strong>insight</strong> is still in development, so be sure to report any bugs and provide feedback for improvement at our <a href=\"https://github.com/bitpay/insight-ui/issues\" target=\"_blank\">github issue tracker</a>.":"<strong>인사이트</strong> 는 아직 개발중입니다. 새로운 요구사항이나 버그가 발견되면 저희의 <a href=\"https://github.com/bitpay/insight/issues\" target=\"_blank\">깃허브이슈 트래커r</a> 에 정보를 제공해주시기 바랍니다.","About":"인사이트에 대하여","Address":"주소","Age":"생성후 경과 시간","An error occured in the verification process.":"검증 프로세스중 오류가 발생했습니다.","An error occured:<br>{{error}}":"오류발생:<br>{{error}}","Application Status":"상태","{{APP_CONFIG.COINNAME}} comes with a way of signing arbitrary messages.":"{{APP_CONFIG.COINNAME}} 임의의 메시지에 서명하는 방법을 제공합니다.","{{APP_CONFIG.COINNAME}} node information":"{{APP_CONFIG.COINNAME}} 노드 정보","Block":"블럭","Block Reward":"블럭 보상","Blocks":"블럭","Can't connect to bitcoind to get live updates from the p2p network. (Tried connecting to bitcoind at {{host}}:{{port}} and failed.)":"p2p 네트웍으로부터 동기화하기 위해서 bitcoind 에 연결할수 없습니다.(bitcoind ({{host}}:{{port}} 연결중 실패했습니다.)","Can't connect to insight server. Attempting to reconnect...":"insight server 에 연결할수 없습니다. 재시도중...","Can't connect to internet. Please, check your connection.":"인터넷에 연결할수 없습니다. 네트웍 연결을 확인하시기 바랍니다.","Complete":"완료","Confirmations":"승인됨","Conn":"연결","Connections to other nodes":"연결된 노드","Current Blockchain Tip (insight)":"최신 블럭체인 Tip (insight)","Current Sync Status":"현재 동기화 상태","Details":"자세히","Difficulty":"난이도","Double spent attempt detected. From tx:":"이중 지불이 감지되었습니다. From tx:","Error message:":"오류 메시지:","Error!":"오류!","Fee":"수수료","Final Balance":"최종 잔액","Finish Date":"완료 일자","Go to home":"Home 으로","Height":"높이","Included in Block":"블럭에 포함된","Info Errors":"오류 정보","Initial Block Chain Height":"시작 블럭체인 높이","Input":"입력","Last Block":"최종 블럭","Last Block Hash (Bitcoind)":"최종 블럭 해시 (Bitcoind)","Latest Blocks":"최신 블럭들","Latest Transactions":"최신 거래들","Loading Address Information":"주소정보 읽어오는 중","Loading Block Information":"블럭정보 읽어오는 중","Loading Selected Date...":"선택될 날짜 읽어오는 중...","Loading Transaction Details":"상세 거래 읽어오는 중","Loading Transactions...":"거래들 읽는중...","Loading...":"읽어오는중...","Message":"메시지","Mined Time":"마이닝 시간","Mined by":"채굴자","Mining Difficulty":"마이닝 난이도","Next Block":"다음 블럭","No Inputs (Newly Generated Coins)":"입력없음 (새롭게 생성된 코인)","No blocks yet.":"아직 블럭들이 없습니다.","No matching records found!":"요청한 내역과 일치하는 레코드가 없습니다！","No. Transactions":"포함된 거래수","Number Of Transactions":"포함된 거래수","Output":"출력","Powered by":"Powered by","Previous Block":"이전 블럭","Protocol version":"프로토콜 버전","Proxy setting":"프록시 설정","Raw transaction data":"원시 거래 데이터","Raw transaction data must be a valid hexadecimal string.":"Raw transaction 자료는 옳바른 16진수 숫자 문자열이어야 합니다.","Received Time":"수신 시간","Search for block, transaction or address":"블럭,거래,주소 검색","See all blocks":"모든 블럭 보기","Send transaction":"거래 전송","Show Transaction Output data":"거래 출력 자료 보기","Show all":"전부보기","Show input":"입력 보기","Show less":"조금 보기","Show more":"좀더 보기","Signature":"서명","Size":"크기","Size (bytes)":"크기 (bytes)","Skipped Blocks (previously synced)":"건너뛴 블럭들 (이전에 동기화된)","Start Date":"시작 일자","Status":"상태","Summary":"요약","Summary <small>confirmed</small>":"요약 <small>승인됨</small>","Sync Progress":"동기화 진행중","Sync Status":"동기화 상태","Sync Type":"동기화 종류","Synced Blocks":"동기화된 블럭","The message failed to verify.":"메시지 검증에 실패했습니다.","The message is verifiably from {{verification.address}}.":"이 메시지는 {{verification.address}} 으로 부터 검증되었습니다.","There are no transactions involving this address.":"이 주소는 거래내역을 포함하고 있지 않습니다","This form can be used to broadcast a raw transaction in hex format over the {{APP_CONFIG.COINNAME}} network.":"이 양식은 헥사값으로 이뤄진 raw transaction 을 {{APP_CONFIG.COINNAME}} 에 전파하기 위해서 이용됩니다. ","This form can be used to verify that a message comes from a specific {{APP_CONFIG.COINNAME}} address.":"이 양식을 사용하면 특정 {{APP_CONFIG.COINNAME}} 주소에서 메시지가 왔는지 확인할 수 있습니다. ","Time Offset":"시간 Offset","Timestamp":"타임스템프","Today":"오늘","Total Amount":"종 자산","Total Received":"총 수신","Total Sent":"총 전송","Transaction":"거래","Transaction Output Set Information":"거래 출력 정보","Transaction Outputs":"거래 출력들","Transaction succesfully broadcast.<br>Transaction id: {{txid}}":"거래 내역이 성공적으로 전파되었습니다 <br>거래 id: {{txid}}","Transactions":"거래들","Type":"종류","Unconfirmed":"미승인","Unconfirmed Transaction!":"미승인 거래!","Unconfirmed Txs Balance":"미승인 잔고","Verify":"검증","Verify signed message":"서명 메시지 검증","Version":"버전","Waiting for blocks...":"블럭 기다리는 중...","Waiting for transactions...":"거래 기다리는 중...","by date.":"일자별","mined":"체굴일시","mined on:":"일자","Fee Rate":"수수료율","Max Coin":"최대 코인수","Current mined Coins":"현재까지 채굴된 코인수","Remind Coins":"채굴가능 코인수"});
/* jshint +W100 */
}]);